...
#                cv2.line(im0, start_point2, end_point2, (0,255,0), 1, cv2.LINE_AA)
                ...
                cv2.putText(im0, 'Bus: ' + str(num_bus), (10,100), font,0.7, color_green, 1, cv2.LINE_AA)
                cv2.putText(im0, 'Motorcycle: ' + str(num_motor), (10,120), font,0.7, color_green, 1, cv2.LINE_AA)
                cv2.putText(im0, 'Bicycle: ' + str(num_bicycle), (10,140), font,0.7, color_green, 1, cv2.LINE_AA)
...
def count_vehicles(w_obj,h_obj,w,h,id,name):
    global total, listId, num_car, num_truck, num_bus, num_bicycle, num_walker, num_motor
    if h_obj < (h-x_line_pixel) and h_obj > (h-x_line_pixel-30):
        if id not in listId:
                if name == 'car': num_car += 1
                if name == 'bus': num_bus += 1
                if name == 'truck': num_truck += 1
                if name == 'motorcycle': num_motor += 1
                if name == 'bicycle': num_bicycle += 1
                listId.append(id)
